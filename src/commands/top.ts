...
async function getUserList(guildID: string) {
    let userdatas: Types.UserData[] = await Schemas.UserData.find({
            guildID: guildID
        })
        .exec();
    let uDataMid: any = {}
    let userdataSorted: Types.UserData[] = []
    userdatas.forEach(uData => {
        uDataMid[uData.pointsModule.points.toString()] = uData
    });
    let sortedkeys = Object.keys(uDataMid).sort(function (a: string, b: string) {
    ...
    async execute(message: Discord.Message, args: string[]) {
        ...
        for (let index = 0; index < 10; index++) {
            ...
            if (message.guild.members.has(tUserData.userID)) {
                let tUser: any = message.guild.members.get(tUserData.userID)
        ...
        tMessage.push("-------------------------------------")
        let index = 0
        userDataList.forEach((userData: Types.UserData) => {
            index++
